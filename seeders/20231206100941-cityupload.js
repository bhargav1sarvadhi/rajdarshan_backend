...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        const data = [];
        const headers = ['id', 'name', 'state_id'];
        await new Promise((resolve, reject) => {
            fs.createReadStream(csvFilePath)
                .pipe(csv({ headers }))
                .on('data', (raw) => {
                    const { id, name, state_id } = raw;
                    data.push({
                        id,
                        name,
                        state_id,
        ...
        return queryInterface.bulkInsert('cities_details', data, {});
    },
...
//         try {
//             const files = ['../src/uploads/cities_details.json'];
