...
export async function uploadVideo(filename, video_data) {
    try {
        const file = `${process.env.FOLDER_NAME}/${filename}`;
        const upload = {
            Bucket: bucket,
            Key: file,
            Body: video_data,
...
export async function upload_image(filename, data) {
    try {
        const file = `${process.env.FOLDER_NAME}/${filename}`;
...
export const deleteImageAWS = (key) => {
    const image = `${process.env.FOLDER_NAME}/${key}`;
    const params = {
        Bucket: bucket,
        Key: image,
    };
...
export const renameImageAWS = async (oldKey) => {
    const image = `${process.env.FOLDER_NAME}/${oldKey}`;

    await s3
        .copyObject({
            Bucket: bucket,
            CopySource: `/${bucket}/${image}`,
            Key: `${process.env.FOLDER_NAME}/default-image1.jpg`,
        })
        .promise();
    return true;
};
